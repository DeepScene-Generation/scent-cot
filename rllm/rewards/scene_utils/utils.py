...
from functions import get_above_constraint, get_behind_constraint, get_in_corner_constraint, get_in_front_constraint, get_left_of_constraint, get_right_of_constraint, get_on_constraint, get_under_contraint
...
def normalize_layout_element_id(layout_element_id):
    ...
    return replacements.get(layout_element_id, layout_element_id)
...
def get_possible_positions(object_id, scene_graph, room_dimensions):
    ...
    func_map = {
        ...
        "left_of" : get_left_of_constraint,
        "right of" : get_right_of_constraint,
        "right_of" : get_right_of_constraint,
        "to the right of": get_right_of_constraint,
        "to the left of": get_left_of_constraint,
        "in front" : get_in_front_constraint,
        "in front of" : get_in_front_constraint,
        "in_front_of" : get_in_front_constraint,
        "behind" : get_behind_constraint,
        "behind_of" : get_behind_constraint,
        "behind of" : get_behind_constraint,
        "above" : get_above_constraint,
        "in the corner" : get_in_corner_constraint,
        "in_corner" : get_in_corner_constraint,
        "in the middle of" : get_on_constraint,
        "in the middle": get_on_constraint,
        "in_the_corner": get_in_corner_constraint,
        "in the middle of the room": get_on_constraint,
        "on_top_of": get_on_constraint,
        "on top of": get_on_constraint,
    }
    ...
    for constraint in constraints:
        ...
        cons_name = constraint[key]
        if cons_name in ["middle_of_the_room", "north wall", "west wall", "east wall", "south wall"]:
            cons_name = normalize_layout_element_id(cons_name)
        if cons_name in ["floor"]:
            continue
        if prep == "in":
            prep = "on"
        obj_B = [element for element in scene_graph if element.get("new_object_id") == cons_name][0]
...
def check_constraints(scene_graph, room_dimensions=[5.0, 5.0, 3.0]):
    """
...
if __name__ == "__main__":
    ...
    # 设置房间尺寸
    # room_dimensions = [4, 3, 2.8]  # 可以根据实际房间尺寸调整
    room_dimensions = [5.0, 5.0, 3.0]
    
    # 调用检查函数
    violation_ratio = check_constraints(scene_graph, room_dimensions)
