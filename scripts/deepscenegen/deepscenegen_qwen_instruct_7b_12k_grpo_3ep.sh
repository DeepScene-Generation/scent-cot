...
if [ -z "$MODEL_PATH" ]; then
    MODEL_PATH="checkpoints/deepscene/deepscene-qwen-instruct-7b-3epoch/global_step_165"
fi

# Train over 4 nodes, 8 A100-80GB GPUs per node.
python3 -m verl.trainer.main_ppo \
    algorithm.adv_estimator=grpo \
    data.train_files=data/train_scenecot_rl.parquet \
    data.val_files=data/test_scenecot.parquet \
