...
class RLHFDataset(Dataset):
    ...
    def __getitem__(self, item):
        ...
        chat = row_dict.pop(self.prompt_key).item()['content']
        ...
        row_dict["extra_info"]["user"] = user_preference
        ...
        if self.use_system_prompt and self.use_system_prompt == "scene_cot":
            self.system_prompt = SCENECOT_SYSTEM_PROMPT
            prompt_chat = [
                {'role': 'system', 'content': self.system_prompt},
                {'role': 'user', 'content': chat}
            ]
        else:
            prompt_chat = [{'role': 'user', 'content': chat}]
            assert False

        

        prompt_with_chat_template = self.tokenizer.apply_chat_template(prompt_chat, add_generation_prompt=True, tokenize=False)
        # print("prompt_with_chat_template=", prompt_with_chat_template)
        # assert False
        ...
        row_dict["data_source"] = "scene_cot"
